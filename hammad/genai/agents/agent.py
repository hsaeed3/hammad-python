...
class AgentSettings:
    ...
    context_updates: Optional[Union[List[Literal["before", "after"]], Literal["before", "after"]]] = field(default=None)
...
class AgentModelSettings(BaseGenAIModelSettings):
    ...
    # Context management settings
    context_updates: Optional[Union[List[Literal["before", "after"]], Literal["before", "after"]]] = None
    """When to update context ('before', 'after', or both)."""
...
class Agent(BaseGenAIModel, Generic[T]):
    ...
    def _should_update_context(self, context: AgentContext, timing: Literal["before", "after"]) -> bool:
    ...
    def _create_context_update_model(self, context: AgentContext, field_name: str = None):
    ...
    def _perform_context_update(self, context: AgentContext, model: LanguageModel, current_messages: List[Dict[str, Any]], timing: Literal["before", "after"]) -> AgentContext:
    ...
    def _format_messages_with_context(
        ...
        if self.instructions:
            ...
            if context is not None:
                context_str = _format_context_for_instructions(context, self.context_format)
                if context_str:
                    system_content += f"\n\nContext:\n{context_str}"
