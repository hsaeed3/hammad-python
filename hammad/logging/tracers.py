...
Contains very helpful decorators for tracing the execution of
...
from .logger import Logger, get_logger, LoggingLevelName
from .._builtins._internal._style_types import (
    StyleColorName,
    StyleStyleSettings,
...
def trace_function(
    ...
    *,
    ...
    level: Union[LoggingLevelName, str, int] = "debug",
    rich: bool = True,
    style: Union[StyleColorName, StyleStyleSettings, str] = "white",
    bg: Union[StyleColorName, StyleBackgroundSettings, str] = None,
    indent: int = 2,
    ...
    def decorator(target_fn: Callable[_P, _R]) -> Callable[_P, _R]:
        ...
        def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> _R:
            ...
            if logger is None:
                ...
                _logger = get_logger(name=logger.name)
            ...
            if parameters:
                ...
                if param_info:
                    entry_msg += f"\n{indent_str}Parameters: {', '.join(param_info)}"
            ...
            try:
                ...
                if rich:
                    ...
                    error_msg += (
                        f"\n{indent_str}[red]{type(e).__name__}: {str(e)}[/red]"
                    )
                else:
                    error_msg = f"✗ Exception in {module_name}.{func_name}() (after {exec_time:.3f}s)"
                    error_msg += f"\n{indent_str}{type(e).__name__}: {str(e)}"
    ...
    def decorator(target_cls: Type[Any]) -> Type[Any]:
        ...
        def traced_init(self, *args, **kwargs):
            ...
            if attributes:
                ...
                if attr_info:
                    if rich:
                        attr_msg = f"[{style}]{indent_str}Initial attributes: {', '.join(attr_info)}[/{style}]"
                    else:
                        attr_msg = (
                            f"{indent_str}Initial attributes: {', '.join(attr_info)}"
                        )
...
def trace(
    *,
    ...
    [Union[Callable[_P, _R], Type[Any]]], Union[Callable[_P, _R], Type[Any]]
]: ...
...
def trace(
    ...
    *,
    ...
    1. @trace (direct decoration)
    2. @trace() (parameterized with defaults)
    3. @trace(parameters=["x"], level="info") (parameterized with custom settings)

    When applied to a function, it logs entry/exit and optionally tracks parameters.
    When applied to a class, it can track attribute changes and trace specific methods.

    Args:
        func_or_cls: The function or class to trace (when used directly)
        parameters: For functions, the parameters to log
        attributes: For classes, the attributes to track changes
        functions: For classes, the methods to trace
    ...
    if func_or_cls is None:
        # Called with parameters: @trace(parameters=["x"])
        return decorator
    else:
        # Called directly: @trace
