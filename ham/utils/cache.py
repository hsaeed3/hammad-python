...
def make_cache_key(*args, **kwargs) -> str:
    ...
    def _hash_obj(obj: Any) -> str:
        ...
        if obj is None:
            ...
            pairs = [f"{_hash_obj(k)}:{_hash_obj(v)}" 
...
# Convenience aliases for specific cache types
def ttl_cache(ttl: float, maxsize: int = 128, **kwargs) -> Callable[[F], F]:
    """Create a TTL cache decorator."""
    return lambda f: cached(f, ttl=ttl, maxsize=maxsize, **kwargs)


def file_cache(path: Union[str, Path], maxsize: int = 128, **kwargs) -> Callable[[F], F]:
    """Create a persistent file cache decorator."""
    return lambda f: cached(f, path=path, maxsize=maxsize, **kwargs)


def lru_cache(maxsize: int = 128, **kwargs) -> Callable[[F], F]:
    """Create an LRU cache decorator (alias for cached)."""
    return lambda f: cached(f, maxsize=maxsize, **kwargs)
