"""ham._internal._type_checking_importer"""
...
class TypeCheckingImporterError(Exception):
    """An error that occurs when the `_type_checking_importer` function
    fails to create a lazy loader function."""


class TypeCheckingImporterCache:
    """Minimal cache implementation for internal use only
    within the `_type_checking_importer` function."""
...
# NOTE:
# SINGLETON
GETATTR_IMPORTER_PARSE_CACHE = TypeCheckingImporterCache(maxsize=64)
...
def _parse_type_checking_imports(source_code: str) -> dict[str, tuple[str, str]]:
    ...
    This is used to create the mapping used within the `_type_checking_importer_from_import_dict`
    function.
...
def _type_checking_importer_from_import_dict(
    ...
    def __getattr__(name: str) -> Any:
        ...
        raise TypeCheckingImporterError(f"module '{package}' has no attribute '{name}'")

    return __getattr__


def type_checking_importer(
    all: Union[Tuple[str, ...], List[str]],
) -> Callable[[str], Any]:
    """Loader used internally within the `ham` package to create lazy
    ...
    Example:
        ...
        from ham.performance.imports import _type_checking_importer
        ...
        __getattr__ = _type_checking_importer(all)
    ...
    loader = _type_checking_importer_from_import_dict(filtered_map, package, all)
    GETATTR_IMPORTER_TYPE_CHECKING_CACHE[cache_key] = loader
    return loader


__all__ = ("type_checking_importer",)