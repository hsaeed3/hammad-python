...
def trace_http(
    ...
    *,
    ...
    def _log_request_info(bound_args: inspect.BoundArguments, func_name: str, module_name: str, _logger: Logger):
    ...
    def _log_response_info(result: Any, exec_time: float, func_name: str, module_name: str, _logger: Logger):
    ...
    def _log_error_info(e: Exception, exec_time: float, func_name: str, module_name: str, _logger: Logger):
        ...
        if rich:
            ...
            error_msg = f"❌ HTTP Error in {module_name}.{func_name}() (after {exec_time:.3f}s)"
            error_msg += f"\n  {type(e).__name__}: {str(e)}"

        _logger.error(error_msg)

    def decorator(target_fn: Union[Callable[_P, _R], Callable[_P, Awaitable[_R]]]) -> Union[Callable[_P, _R], Callable[_P, Awaitable[_R]]]:
        ...
        if asyncio.iscoroutinefunction(target_fn):
            ...
            async def async_wrapper(*args: _P.args, **kwargs: _P.kwargs) -> _R:
                _logger = _get_logger(f"http.{target_fn.__module__}.{target_fn.__name__}")
                ...
                try:
                    ...
                    # Log response info
                    _log_response_info(result, exec_time, func_name, module_name, _logger)
...
def install_trace_http(
    *,
    ...
    patches = [
        ...
        # litellm - patch the main completion function
    ...
    if patched_functions:
        _logger = logger if isinstance(logger, Logger) else create_logger(
            name="http.install_trace", level=level, rich=rich
        )
        ...
        if rich:
            ...
            install_msg = f"✨ Installed HTTP tracing on {len(patched_functions)} functions"
            install_msg += f"\n  " + "\n  ".join(patched_functions)
        ...
        _logger.warning("No HTTP functions found to patch. Import HTTP libraries first.")