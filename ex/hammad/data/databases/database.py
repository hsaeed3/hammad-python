...
class Database(Generic[DatabaseEntryType]):
    ...
    def __init__(self, location: DatabaseLocation = "memory", path: str = "database.db", default_ttl: Optional[int] = None):
        """
    ...
    def _get_from_file(self, id: str, collection: str, filters: Optional[Filters] = None) -> Optional[DatabaseEntryType]:
    ...
    def _add_to_file(self, entry: DatabaseEntryType, id: Optional[str], collection: str, filters: Optional[Filters], ttl: Optional[int]) -> None:
        ...
        with self._SessionLocal() as session:
            ...
            existing = session.execute(select(table).where(table.c.id == item_id)).fetchone()
            
            if existing:
                # Update existing item
                stmt = update(table).where(table.c.id == item_id).values(
    ...
    def _query_from_file(self, collection: str, filters: Optional[Filters], search: Optional[str], limit: Optional[int]) -> List[DatabaseEntryType]:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<Database {location_info} collections={list(all_collections)}>"
        )
    ...
    def create_vector_collection(
        ...
        *,
        ...
        elif self.location == "file" and "path" not in qdrant_config and "host" not in qdrant_config:
