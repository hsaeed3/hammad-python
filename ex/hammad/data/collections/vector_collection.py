...
class VectorCollection(BaseCollection, Generic[Object]):
    ...
    def __init__(
        ...
        Args:
            ...
            api_base: Optional API base URL for remote models
            api_key: Optional API key for remote models
        """
        self.name = name
        ...
        if model:
            self._embedding_model = EmbeddingModel(model)
            self._embedding_function = self._create_embedding_function(model, api_base, api_key)
    ...
    def _create_embedding_function(self, model_name: str, api_base: Optional[str] = None, api_key: Optional[str] = None) -> Callable[[Any], List[float]]:
        """Create an embedding function from a model name."""
        def embedding_function(text: Any) -> List[float]:
            if not isinstance(text, str):
                text = str(text)
            
            # Use the sync embedding method with optional API parameters
            kwargs = {"model": model_name, "input": [text]}
            if api_base:
                kwargs["api_base"] = api_base
            if api_key:
                kwargs["api_key"] = api_key
            
            response = EmbeddingModel.embed(**kwargs)
