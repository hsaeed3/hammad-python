...
class TestMCPServerService:
    ...
    def test_generate_runner_script_basic(self):
        ...
        script = service._generate_runner_script(
            ...
            tools_source_code=["def dummy_tool(param: str) -> str:\n    return f'Result: {param}'"],
    ...
    def test_verify_process_started_timeout(self, mock_sleep):
        ...
        with patch('time.time') as mock_time:
            mock_time.side_effect = [0, 0.2, 0.3, 0.4, 0.5]  # Extra values for logging calls
    ...
    def test_cleanup_single_process_graceful_termination(self):
        ...
        service._cleanup_single_process(mock_process, "test_server", force_kill_timeout=1.0)
    ...
    @patch('subprocess.Popen')
    @patch('inspect.getsource')
    def test_launch_server_process_verification_failure(self, mock_getsource, mock_popen):
